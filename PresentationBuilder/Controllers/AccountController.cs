		...
		public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
		{
			...
			if (ModelState.IsValid)
			{
				...
				else
				{
					...
					var aspNetUserId = (from u in context.AspNetUsers where u.UserName == model.Email select u.Id).First();

					var result = await UserManager.AddLoginAsync(aspNetUserId, info.Login);
					return result.Succeeded ? RedirectToAction("Index", "Presentations") : RedirectToAction("ExternalLoginFailure");
				}
